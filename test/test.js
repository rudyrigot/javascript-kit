(function(Prismic) {
  ...
  asyncTest('StructuredTexts asHtml handles list issues', 1, function() {
    Prismic.Api('https://mobicorp.prismic.io/api', function(err, Api) {
      if (err) { console.log(err); return; }
      Api.form('everything').query('[[:d = at(document.id, "U2ucHgEAAC4ALRo1")]]').ref(Api.master()).submit(function(err, documents) {
        if (err) { console.log(err); return; }
        equal(documents.results[0].asHtml(ctx).replace(/\n/g, ''), '<section data-field=\"knowledge_center_blog_post.title\"><h1>What\'s new in mobi.Route</h1></section><section data-field=\"knowledge_center_blog_post.description\"><h2>Release notes for mobi.Route</h2></section><section data-field=\"knowledge_center_blog_post.content\"><p><strong>Address geocoding improvements:</strong> We have enhanced our address geocoding engine which results in fewer invalid addresses when creating a plan in mobi.Route.</p><p><strong>Add a new visit:</strong> There is a new button on the plan review page to add a new visit after a plan has been run. When a new visit is added, it will show up in the unassigned section. The visit can then be assigned to any driver’s route by clicking the move visit icon. If a new visit is added and assigned to a driver’s route, the new visit will show up on the driver’s mobi.Connect mobile application immediately after the plan is committed.</p><p><strong>Add a new route:</strong> There is a new button on the plan review page to add a new route after a plan has been run. When a new route is added, it will show up as a new color with an empty list of visits. You can then move visits from any other route to the newly created route.</p><p><strong>Move a visit to a different route:</strong> There is a new icon next to each visit to move that visit to a different route. When moving a visit, specify the route to move the visit to, as well as the sequence for the visit in the new route. Visits can also be moved to and from the unassigned section. If a visit is moved to a new driver’s route, the affected driver’s mobi.Connect mobile applications will be updated immediately after the plan is committed.</p><p><strong>Delete a visit:</strong> There is a new icon next to each visit to delete that visit from the plan. If you delete a visit, it will be removed from the mobi.Route application completely. If a visit is deleted, the visit will be removed from the driver’s mobi.Connect mobile application immediately after the plan is committed.</p><p>If you have any questions about these new features, please contact your local mobi sales rep or send an email to sales-team@mobicorp.com.</p><p>Thanks!The mobi team</p></section><section data-field=\"knowledge_center_blog_post.categories\"><section data-field=\"category\"><a href=\"/testing_url/U2r0qgEAAC0AK8qd/customers?ref=XXXXX\">/testing_url/U2r0qgEAAC0AK8qd/customers?ref=XXXXX</a></section></section><section data-field=\"knowledge_center_blog_post.products\"><section data-field=\"product\"><a href=\"/testing_url/U2eWegEAADAARUwe/route?ref=XXXXX\">/testing_url/U2eWegEAADAARUwe/route?ref=XXXXX</a></section></section><section data-field=\"knowledge_center_blog_post.content_type\"><a href=\"/testing_url/U2eWBgEAAC0ARUsc/release-notes?ref=XXXXX\">/testing_url/U2eWBgEAAC0ARUsc/release-notes?ref=XXXXX</a></section><section data-field=\"knowledge_center_blog_post.date\"><time>Mon May 05 2014 17:00:00 GMT-0700 (PDT)</time></section>');
        start();
      });
    });
  });
